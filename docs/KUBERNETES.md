...
gateway-api-key
webui-secret-key
memory-subject-hmac-key
continuity-api-token
...
`memory-subject-hmac-key` is strongly recommended and is generated by the local Hub workflow.
If it is absent during an existing-Secret upgrade, the gateway temporarily derives pseudonymous
subjects with its gateway key for compatibility. `continuity-api-token` is required only when the
selected memory provider uses `continuity-http`; both keys are optional Secret references so a
built-in-memory upgrade does not prevent the gateway Pod from starting.
