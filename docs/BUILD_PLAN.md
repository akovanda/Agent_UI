...
- repository-ready Git history, release tag, CI, and reproducible archives;
...
- `./hub test` passes with branch-aware coverage at or above 80 percent;
- all YAML and Python parse;
- archive contains no generated secrets or model weights;
- repository passes `./hub lint` and contains no tracked secrets or model weights.
...
5. Run `./hub init`, model verification, and `./hub doctor --gpu`.

Exit criteria:

- `./hub doctor --gpu` reports no fatal issue;
...
4. Exercise `hermes-agent` at 64K target.
...
- Open WebUI and all SillyTavern named volumes are included in backup jobs.
...
1. Start Hermes with `./hub up --agent` and verify its API and dashboard health.
2. Add Hermes as a second Open WebUI OpenAI connection.
3. Verify `hermes-agent` context actually remains at or near 64K on the T4.
...
1. Publish the reviewed repository and protect its default branch.
