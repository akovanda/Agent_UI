...
Initialize 0.2.0 first:

```bash
./hub init
```

For a parallel validation deployment, edit the generated mode-0600 `.env` before starting any
containers. Set a distinct `COMPOSE_PROJECT_NAME` and distinct values for every `*_VOLUME` entry.
The generated high host ports are already independent, or they can be regenerated with
