...
def cmd_model_register(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "no_profile", False):
        profile_id = getattr(args, "profile_id", None) or args.model_id
        if not MODEL_ID_RE.fullmatch(profile_id):
            raise HubError(f"Invalid profile id: {profile_id}")
        profiles = overlay.setdefault("profiles", {})
        defaults = (
            {
                "temperature": 1.2,
                "top_p": 0.95,
        ...
        profiles[profile_id] = {
            ...
            "advertised": True,
            "description": (
                args.description or f"Direct access to {args.display_name or args.model_id}."
            ),
            **defaults,
        }

    atomic_write(overlay_path, yaml.safe_dump(overlay, sort_keys=False), mode=0o600)
    message = f"Registered {args.model_id} in {overlay_path}"
    if not getattr(args, "no_profile", False):
        profile_id = getattr(args, "profile_id", None) or args.model_id
        message += f" with selectable profile {profile_id}"
    print(message)
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    model_register.add_argument(
        "--profile-id",
        help="Selectable gateway profile id (defaults to MODEL_ID)",
    )
    model_register.add_argument(
        "--no-profile",
        action="store_true",
        help="Register the inference model without creating a gateway profile",
    )
